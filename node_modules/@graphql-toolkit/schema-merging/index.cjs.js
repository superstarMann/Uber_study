...
function mergeArguments(args1, args2, config) {
    ...
    if (config && config.sort) {
        result.sort(common.compareNodes);
    }
    return result;
}
...
function preventConflicts(type, a, b, ignoreNullability = false) {
    ...
    if (common.isNotEqual(aType, bType)) {
...
function mergeGraphQLTypes(types, config) {
    ...
    const allNodes = types
        ...
        if (graphql.isSchema(type)) {
            return graphql.parse(common.printSchemaWithDirectives(type));
        }
    ...
    const schemaDefinition = common.createSchemaDefinition(schemaDef, {
...
function mergeSchemas(config) {
    ...
    for (const schema of config.schemas) {
        extractedResolvers.push(common.extractResolversFromSchema(schema));
        extractedExtensions.push(extractExtensionsFromSchema(schema));
    }
    extractedResolvers.push(...ensureResolvers(config));
    const resolvers = common.composeResolvers(mergeResolvers(extractedResolvers, config), config.resolversComposition || {});
...
async function mergeSchemasAsync(config) {
    const [typeDefs, resolvers, extensions] = await Promise.all([
        mergeTypes(config),
        Promise.all(config.schemas.map(async (schema) => common.extractResolversFromSchema(schema))).then((extractedResolvers) => common.composeResolvers(mergeResolvers([...extractedResolvers, ...ensureResolvers(config)], config), config.resolversComposition || {})),
...
function mergeTypes({ schemas, typeDefs, ...config }) {
    return mergeTypeDefs([...schemas, ...(typeDefs ? common.asArray(typeDefs) : [])], config);
}
function ensureResolvers(config) {
    return config.resolvers ? common.asArray(config.resolvers) : [];
}
